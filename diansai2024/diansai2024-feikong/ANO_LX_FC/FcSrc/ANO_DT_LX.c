...
void ANO_DT_Init(void)
{
	...
	dt.fun[0xf1].fre_ms = 10;	  //触发发送的周期100ms
...
static void Add_Send_Data(u8 frame_num, u8 *_cnt, u8 send_buffer[])
{
		...
		case 0xf1: //CHECK返回
	{
		...
		temp_data = (s16)(base_link.speed_x_cms *1); 
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
		
		temp_data = (s16)(base_link.speed_y_cms *1);  
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
	
			
	/*	temp_data = (s16)(ano_of.of1_dx *1);  
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
		
				temp_data = (s16)(ano_of.of1_dy *1);  
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);*/

		//ano_of.of1_dx
		//ano_of.of1_dy
		temp_data_32 = (s32)(base_link.pose_x_cm * 1);
		send_buffer[(*_cnt)++] = BYTE0(temp_data_32);
		send_buffer[(*_cnt)++] = BYTE1(temp_data_32);
		send_buffer[(*_cnt)++] = BYTE2(temp_data_32);
		send_buffer[(*_cnt)++] = BYTE3(temp_data_32);
		
		temp_data_32 = (s32)(base_link.pose_y_cm * 1);
		send_buffer[(*_cnt)++] = BYTE0(temp_data_32);
		send_buffer[(*_cnt)++] = BYTE1(temp_data_32);
		send_buffer[(*_cnt)++] = BYTE2(temp_data_32);
		send_buffer[(*_cnt)++] = BYTE3(temp_data_32);
		
		send_buffer[(*_cnt)++] = BYTE0(ok_flag);
	//	led_flag++;
		send_buffer[(*_cnt)++]=  BYTE0( led_flag);
